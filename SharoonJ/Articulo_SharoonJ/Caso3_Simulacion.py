...
for n in range(Nt):
    ...
    if n == impact_step:
        ...
        # Pulso gaussiano central
        impacto = np.exp(
            -(X**2 + Y**2)/(2*sigma**2)
        )

        # Velocidad inicial preferente
        v0 = 2.0
        ...
        u_current = impacto.copy()

        # Genera movimiento hacia +x
        u_old = impacto + dt*v0*du_dx
    ...
    if n % 25 == 0:
        ...
        ax1.set_title(
            f"Caso 3 - Impacto Oblicuo | t = {tiempo:.2f}"
        )
